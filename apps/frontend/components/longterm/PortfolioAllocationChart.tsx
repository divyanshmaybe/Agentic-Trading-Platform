				...
				{finalPortfolio.map((item, index) => {
					...
					return (
						<div
							...
							className="group flex items-center justify-between rounded-xl border border-white/10 bg-white/8 px-4 py-3 text-sm hover:bg-white/10 transition-colors backdrop-blur-sm"
						>
							<div className="flex items-center gap-3">
								...
								<div className="flex flex-col">
									...
									{item.reasoning && (
										<span className="text-lg text-white/60 line-clamp-1 opacity-0 max-h-0 group-hover:opacity-100 group-hover:max-h-20 transition-all duration-300 ease-in-out overflow-hidden">
											{item.reasoning}
										</span>
									)}
								</div>
							</div>
							<div className="text-right text-xl font-semibold text-emerald-400">
