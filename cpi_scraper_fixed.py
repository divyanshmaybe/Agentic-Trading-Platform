...
class CPIScraperFixed:
    ...
    def establish_session(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                ...
                verify=False,
                timeout=30,
                allow_redirects=True  # Follow redirects to get session cookie
            )
    ...
    def get_form_data(self) -> dict:
        ...
        try:
            ...
            form_data = {}
            form_fields = ['__VIEWSTATE', '__VIEWSTATEGENERATOR', '__EVENTVALIDATION', '__VIEWSTATEENCRYPTED']
            
            for field in form_fields:
                element = soup.find('input', {'name': field})
                if element and element.get('value'):
                    form_data[field] = element['value']
                    value_preview = element['value'][:50] if len(element['value']) > 50 else element['value']
    ...
    def scrape_all_states(self,
                         ...
                         to_year: str = None,
                         to_month: str = None,
                         group_code: str = "27") -> Optional[pd.DataFrame]:
        ...
        print("\n" + "="*80)
        print("CPI DATA SCRAPER - ALL STATES (FIXED)")
        print("="*80)
        print(f"Period: {from_month}/{from_year} to {to_month}/{to_year}")
        print(f"States: ALL ({len(self.ALL_STATES)} states/UTs)")
        ...
        # Prepare POST data with ALL states selected
        ...
        # Add ALL state checkboxes
        for idx, (state_name, state_code) in enumerate(self.ALL_STATES.items()):
            checkbox_name = f'ctl00$Content1$CheckBoxList1${idx}'
            post_data[checkbox_name] = state_code
        
        print(f"✓ Selected all {len(self.ALL_STATES)} states")
